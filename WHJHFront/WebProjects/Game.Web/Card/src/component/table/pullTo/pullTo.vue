...
export default {
  ...
  methods: {
    actionPull() {
      ...
      this.direction === 'down' ? (this.block = 'down') : (this.block = 'up')
      this.blockShow(true)
    },
    actionTrigger() {
      this.state = 'trigger'
    ...
    blockShow(flag) {
      flag
        ? this.block === 'down' ? (this.topLoadShow = true) : (this.bottomLoadShow = true)
        : this.block === 'down' ? (this.topLoadShow = false) : (this.bottomLoadShow = false)
    },
    ...
    handleTouchMove(event) {
      this.start = 1
      // 临时变量存储this.distance,确保拉动方向判断正确
      ...
      // 其实滚动位置为顶部，拉动方向向下且有下拉提示信息
      if (this.scrollEl.scrollTop === 0 && this.direction === 'down' && this.isTopBounce) {
