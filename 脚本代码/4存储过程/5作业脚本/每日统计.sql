...
BEGIN
	...
	-- 用户总数
	SELECT @UserTotal=COUNT(UserID) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE IsAndroid=0 AND RegisterDate<@StatsEndTime
	
	-- 充值玩家总数
	SELECT @PayUserTotal=COUNT(UserID) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE UserID IN (SELECT UserID FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.ShareDetailInfo 
	WHERE ApplyDate<@StatsEndTime)
	
	-- 活跃玩家数
	SELECT @ActiveUserTotal=COUNT(UserID) FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.StreamScoreInfo 
	WHERE DateID=@DateID AND OnlineTimeCount>=60*60
	
	-- 玩家流失数
	SELECT @LossUserTotal=COUNT(UserID) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE LastLogonDate<DATEADD(mm,-1,@StatsEndTime) AND IsAndroid=0

	SELECT @LossUser=COUNT(UserID) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE LastLogonDate<DATEADD(mm,-1,@StatsEndTime) AND LastLogonDate>=DATEADD(mm,-1,@StatsStartTime) AND IsAndroid=0

	SELECT @LossPayUserTotal=COUNT(UserID) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE UserID IN (SELECT UserID FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.ShareDetailInfo) 
	AND LastLogonDate<DATEADD(mm,-1,@StatsEndTime) AND IsAndroid=0
	
	SELECT @LossPayUser=COUNT(UserID) FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE UserID IN (SELECT UserID FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.ShareDetailInfo) 
	...
	FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.ShareDetailInfo WHERE ApplyDate<@StatsEndTime
	SET @PayAmountTotal = @PayAmountForCurrency
	-- 金币总数
	SELECT @GoldTotal=ISNULL(SUM(Score+InsureScore),0) FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.GameScoreInfo
	
	-- 货币总数
	SELECT @CurrencyTotal=ISNULL(SUM(Diamond),0) FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.UserCurrency
	WHERE UserID NOT IN (SELECT UserID FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo 
	WHERE IsAndroid=1)

	-- 平均时长
	SELECT @UserAVGOnlineTime=ISNULL(AVG(CONVERT(BIGINT,OnLineTimeCount)),0) 
	FROM WHJHAccountsDBLink.WHJHAccountsDB.dbo.AccountsInfo WHERE IsAndroid=0

	-- 游戏税收总数
	SELECT @GameTaxTotal=ISNULL(SUM(Revenue),0) FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.StreamScoreInfo 
	WHERE DateID<=@DateID

	-- 当日游戏税收和损耗
	SELECT @GameTax=ISNULL(SUM(Revenue),0),@Waste=ISNULL(SUM(Waste),0) FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.RecordDrawInfo
	WHERE ConcludeTime<@StatsEndTime AND ConcludeTime>=@StatsStartTime

	-- 当日银行税收
	SELECT @BankTax=ISNULL(SUM(Revenue),0) FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.RecordInsure
	...
	FROM WHJHTreasureDBLink.WHJHTreasureDB.dbo.RecordDrawInfo 
	...
	FROM WHJHPlatformDBLink.WHJHPlatformDB.dbo.GameRoomInfo
