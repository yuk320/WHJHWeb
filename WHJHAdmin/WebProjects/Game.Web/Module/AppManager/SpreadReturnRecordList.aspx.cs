...
{
    public partial class SpreadReturnRecordList : AdminPage
    {
        #region 事件绑定
        ...
        private void SetCondition()
        {
            ...
            int targetType = Convert.ToInt32(ddlSearchTarget.SelectedValue);
            ...
            if (!string.IsNullOrEmpty(query))
            {
                ...
                condition.AppendFormat(" AND {0}={1} ", 
                    targetType == 1 ? "TargetUserID" : "SourceUserID",
                    type == 1 ? GetUserIDByGameID(Convert.ToInt32(query)) : Convert.ToInt32(query)); 
            }
        ...
        {
            PagerSet pagerSet = FacadeManage.aideRecordFacade.GetList(RecordSpreadReturn.Tablename,
        ...
        public string GetAccountsInfo(int userid)
        {
            ...
            return info != null ? $"<td>{info.GameID} | {info.NickName}</td>" : "<td> </td>";
        }
