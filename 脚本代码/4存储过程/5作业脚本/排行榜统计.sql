...
BEGIN
  ...
  DELETE WHJHNativeWebDB.dbo.CacheWealthRank WHERE DateID = @DateID
	-- 写入财富排行榜
	SELECT * INTO #WealthUser FROM (SELECT TOP 10 ROW_NUMBER() OVER(ORDER BY Diamond DESC,UserID ASC) AS RankNum,* FROM (SELECT A.UserID,A.GameID,A.NickName,A.CustomID,A.FaceID,U.Diamond FROM WHJHTreasureDB.dbo.UserCurrency AS U LEFT JOIN AccountsInfo AS A ON U.UserID = A.UserID AND A.Nullity=0 AND A.IsAndroid=0 AND U.Diamond>0) AS AU WHERE AU.UserID IS NOT NULL) AS A
