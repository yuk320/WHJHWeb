...
namespace Game.Data
{
    ...
    public sealed class AccountsDataProvider : BaseDataProvider, IAccountsDataProvider
    {
        ...
        {
            const string sqlQuery =
                @"SELECT UserID,GameID,SpreaderID,NickName,PassPortID,Compellation,FaceID,CustomID,RegisterOrigin,AgentID,RegisterLogonIP,LastLogonIP FROM AccountsInfo WITH(NOLOCK) WHERE UserID = @UserID";
